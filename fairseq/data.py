...
def load_raw_text_dataset(path, load_splits, src=None, dst=None):
    ...
    for split in load_splits:
        ...
        dataset.splits[split] = LanguagePairDataset(
            IndexedRawTextDataset(src_path, src_dict),   # convert word to idx
...
class LanguagePairDataset(object):
    ...
    def collate_tokens(values, pad_idx, eos_idx, left_pad, move_eos_to_beginning):
        size = max(v.size(0) for v in values)   # longest length of the whole dataset
        res = values[0].new(len(values), size).fill_(pad_idx)   # first, build matrix filled with padding

        def copy_tensor(src, dst):
            assert dst.numel() == src.numel()
            if move_eos_to_beginning:    # first position is eos, used as the first input for decoder
        ...
        for i, v in enumerate(values):
            if left_pad:
                copy_tensor(v, res[i][size-len(v):])  # left (size-len(v)) is padding
            else:
